...
```{r exposure-pathway-reps, warning=FALSE}
table(apcra.total.seem3$Pathway)

```

## Clean Data

```{r expos-pathway-cleanup, warning=FALSE}
...
file.dir <- paste('output/', sep='')
file.name <- paste('/Fig_chem_pchem_funct_use_', Sys.Date(), '.tiff', sep='')
file.path <- paste(file.dir, file.name, sep='')
dir.create(path=file.dir, showWarnings = FALSE, recursive = TRUE)
